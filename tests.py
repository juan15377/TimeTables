...
if __name__ == "__main__":
    ...
    class DummyDB:
        def __init__(self):
            ...
            example_data = [
                (1, "Ingeniería en Sistemas", 1, "A"),
                (2, "Ingeniería en Sistemas", 1, "B"),
                (3, "Ingeniería en Sistemas", 2, "A"),
                (4, "Ingeniería Civil", 1, "A"),
                (5, "Ingeniería Civil", 3, "B"),
                (6, "Medicina", 5, "C"),
            ]
            
             # Agregar más datos para probar con volumen
            for i in range(7, 2000):
                career = ["Ingeniería en Sistemas", "Ingeniería Civil", "Medicina", 
                          "Arquitectura", "Economía", "Derecho"][i % 6]
                semester = (i % 10) + 1
                subgroup = chr(65 + (i % 5))  # A, B, C, D, E
                example_data.append((i, career, semester, subgroup))
    ...
    def on_group_selected(group_id, group_data):
        print(f"Grupo seleccionado: ID={group_id}, Datos={group_data}")
    
    # Crear ventana principal
    with dpg.window(label="Buscador de Grupos", width=600, height=400):
        # Instanciar el selector
        selector = GroupSelector(DummyDB(), on_group_selected)
        selector.setup_ui(dpg.last_item())
    
    dpg.create_viewport(title="Ejemplo de Selector de Grupos", width=800, height=600)
    dpg.setup_dearpygui()
    dpg.show_viewport()
    dpg.start_dearpygui()
    dpg.destroy_context()